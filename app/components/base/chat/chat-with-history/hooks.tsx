...
import { fetchAppMeta, fetchAppParams, fetchConversations } from '@/service'
...
export const useChatWithHistory = (installedAppInfo?: InstalledApp) => {
  ...
  const { data: appParams } = useSWR(['appParams', appId], async () => await fetchAppParams())
  console.log('tiredddd')
  console.log(appParams)
  const { data: appMeta } = useSWR(['appMeta', isInstalledApp, appId], async () => await fetchAppMeta())
  ...
  const checkInputsRequired = useCallback((silent?: boolean) => {
    ...
    if (hasEmptyInput) {
      Toast.notify({ type: 'error', message: t('appDebug.errorMessage.valueOfVarRequired', { key: hasEmptyInput }) })
      return false
    }
